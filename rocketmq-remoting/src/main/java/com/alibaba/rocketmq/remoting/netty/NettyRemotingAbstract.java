...
public abstract class NettyRemotingAbstract {
    ...
    // opaque表示请求发起连接方在同个连接上不同的请求标识代码，每次发送一个消息的时候，
    // 他可以选择同步阻塞的方式和异步非阻塞的方式，不管是哪种方式，他都会保存操作码到ResponseFuture的映射
    protected final ConcurrentHashMap<Integer /* opaque */, ResponseFuture> responseTable =
            new ConcurrentHashMap<Integer, ResponseFuture>(256);

    //可以看到注释，键表示了request code，mq中可以为不同类型的请求码指定不同的处理器Processor处理，
    // 但是要注意消息实际的处理并不是在当前线程，而是被封装成task放到Processor对应的线程池处理：默认处理器是下面的defaultRequestProcessor
    protected final HashMap<Integer/* request code */, Pair<NettyRequestProcessor, ExecutorService>> processorTable =
            new HashMap<Integer, Pair<NettyRequestProcessor, ExecutorService>>(64);

    //信息监听器，监听broker通过channel来的信息进行路由信息的更新
    // 只需监听关闭，异常，空闲状态的信息， 连接信息不在本单元进行监听
    protected final NettyEventExecuter nettyEventExecuter = new NettyEventExecuter();

    //封装了对netty包的处理和部分对nameServer存储的数据查询和删除。是一个消息的默认处理器
    ...
    public RemotingCommand invokeSyncImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis)
        ...
        try {
            ...
            //发消息之后等待反馈，timeoutmillis超时后报超时错误
